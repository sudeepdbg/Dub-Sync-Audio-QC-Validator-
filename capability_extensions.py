...
=========================
Drop-in additions for Audio Alignment Engine v10 (audio_align.py).
...
class TranscriptionEngine:
    ...
    Thread-safe: uses a lock to prevent race conditions during model loading.
    """
    ...
    def _ensure_loaded(self):
        ...
        # Slow path: need to load, use lock
    ...
    def transcribe_sample(self, path, duration_sec=WHISPER_SAMPLE_DURATION_SEC):
        """
...
def check_language_id(path, expected_language=None):
    """
...
def check_profanity(path, wordlist=None):
    """
...
def check_dme_structural(me_path, y_dialogue_ref=None, sr=22050, hop_length=512):
    """
...
def check_audio_description(ffprobe_meta):
    """
...
def get_spatial_loudness(ffmpeg_path, path, target_lufs=ATMOS_SPATIAL_LUFS_TARGET, timeout=60):
    """
    ...
    try:
        ...
        # Try to extract JSON from stderr using multiple strategies
        stats = None
        ...
        # Strategy 2: Look for the full loudnorm JSON output pattern
        if stats is None:
            json_match = re.search(r'\{[\s\S]*?"input_i"[\s\S]*?\}', result.stderr)
            if json_match:
                try:
                    stats = _json.loads(json_match.group())
                except _json.JSONDecodeError:
        ...
        # Strategy 3: Parse line by line looking for key-value pairs
        if stats is None:
            stats = {}
        ...
        if not stats or "input_i" not in stats:
            logger.warning(f"Could not parse loudnorm output for {path}")
            return {}
